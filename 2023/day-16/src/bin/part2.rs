...
fn main() {
    ...
    for i in 0..unsafe{LAVA.len()} {
        ...
        max_length = max_length.max(count_lava());
        reset_lava();
        forward((0, i as i8), SOUTH);
        max_length = max_length.max(count_lava());
        reset_lava();
        forward((i as i8, unsafe{LAVA[0].len()-1} as i8), EAST);
        max_length = max_length.max(count_lava());
        reset_lava();
        forward((unsafe{LAVA.len()-1} as i8, i as i8), SOUTH);
        max_length = max_length.max(count_lava());
        reset_lava();
    }
    println!("{}", max_length);
...
fn reset_lava() {
    let mut new_lava: Vec<Vec<String>> = Vec::new();
    unsafe {
        for line in &LAVA {
